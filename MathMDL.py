...
class MathMDL:
    ...
    def Divide(self):
        if len(self.numbers) > 2 or len(self.numbers) <= 1:
            print("Error dividing. Please pass only two numbers.") 
             ...
             return float(self.numbers[0])/float(self.numbers[1])
    def GetRange(self):
        return float(max(self.numbers))-float(min(self.numbers))
    def GetAverage(self):   
         s = self.Add()
    ...
    def GetMode2(self):
        ...
        mode= 0
        for n in self.numbers:
            if self.numbers.count(n) > count:
                count = self.numbers.count(n)
                mode = n
        return mode
...
class Temperature:
    def __init__(self):
        self.farenheit = 0
        self.celcius = 0
    def FarenheitToCelcius(self):
        return (self.farenheit-32)*(5/9)
    def CelciusToFarenheit(self):
